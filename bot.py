...
Когда вы вносите сумму, нажмите «Подтвердить». Подождите 5-10 минут и сумма будет отражена в кредитах. Тогда вы сможете напрямую взять адрес у бота.⚠️"""
                    ...
                    res_message = """
დაელოდე 5-10 წუთი თანხის ასახვას. მეორედ იგივე კოდზე თანხა არ გააგზავნო! გაითვვალისწინე, რომ ხანდახან ბლოკჩეინი გადატვირთულია და შეიძლება უფრო დიდი დრო დაჭირდეს. დადასტურების შემდეგ დაწერე "/start" რომ იყიდო პროდუქტი.⚠️თანხა არ ისახება? დაწერე "/start" ყოველ 10 წუთში
⛩⛩⛩⛩⛩⛩
Подождите 5-10 минут, пока сумма будет отражена. Не отправляйте деньги на один и тот же код второй раз! Обратите внимание, что иногда блокчейн перегружен и может занять больше времени. После подтверждения введите «/start», чтобы купить продукт.⚠️Деньги не зачислены? Введите «/ start» каждые 10 минут.
"""
                    await call.message.answer(text=res_message)
